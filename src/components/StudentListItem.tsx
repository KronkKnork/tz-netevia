...
export const StudentListItem = ({
	...
	const isFormValid = updatedStudent.fullName !== "" && updatedStudent.birthDate !== "" && updatedStudent.performance !== undefined && ["неуд", "уд", "хор", "отл"].includes(updatedStudent.performance);

	const classCardText = "flex justify-between px-5 mb-2 min-h-[50px]"
	const classInput = "bg-slate-900 border-[#1e1e1e] hover:bg-slate-700 rounded-md p-3 outline-none min-w-[300px]"

	return (
		<li className="bg-slate-800 shadow-lg rounded-xl">
			<div className="flex justify-between bg-slate-700 mb-3 p-1">
				<button onClick={() => setEditing(prev => !prev)} className="text-sky-600 hover:text-sky-500 p-1">
					<FontAwesomeIcon icon={faEdit} className="h-6 w-6" />
				</button>
				<div className="font-bold text-xl">{student.fullName}</div>
				<button onClick={() => deleteStudent(student.id)} className=" text-red-700 hover:text-red-600 p-1">
				...
				<>
					<div className={classCardText}>
						...
						<Input
							...
							onChange={(e) =>
								setUpdatedStudent({ ...updatedStudent, fullName: e.target.value })
							}
						/>
					</div>
					<div className={classCardText}>
					...
					<div className={classCardText}>
						...
						>
							<option value="" disabled>Выберите успеваемость</option>
					...
					<div className="mt-2 mb-5 px-5">
						<button onClick={handleUpdate} disabled={!isFormValid} className={`${isFormValid ? "bg-green-900" : "bg-green-950"} bg-green-900 rounded-md text-white px-5 py-2 mr-4`}>
							Сохранить
						</button>
						<button onClick={() => setEditing(false)} className="bg-red-900 rounded-md text-white px-5 py-2 p-1">
							Отмена
						</button>
					</div>
				</>
